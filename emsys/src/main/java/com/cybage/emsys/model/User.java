package com.cybage.emsys.model;
...
public class User {
    public User(int id, @NotBlank(message = "Name is mandatory to fill !!") @Size(min = 2, max = 30) String name,
			@NotBlank(message = "email is mandatory to fill !!") @Email(regexp = "[a-z0-9._%+-]+@[a-z0-9.-]+\\.[a-z]{2,3}") String email,
			String mobile_number,
			@NotBlank(message = "password is mandatory to fill !!") @Size(min = 8) String password, int status,
			int loginAttempts, int otp, String role) {
		super();
		this.id = id;
		this.name = name;
		this.email = email;
	...
	public String toString() {
		return "User [id=" + id + ", name=" + name + ", email=" + email + ", mobile_number=" + mobile_number
				+ ", password=" + password + ", status=" + status + ", loginAttempts=" + loginAttempts + ", role=" + role + "]";
	}
